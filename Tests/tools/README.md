...
setup → wait for the unmanaged scan → `m` → `?` help → PageDown → esc →
esc quit, asserting on the rendered stream (and that the alternate-screen
restore code is emitted, with no error text anywhere).
