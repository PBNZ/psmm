...
function script:Invoke-PSMMPathDownload {
    ...
    Write-PSMMLine "[$script:PSMM_ColWarn]$($files.Count) cloud-only file(s), $mb MB to download - this can take a while on a slow connection[/]"
    if (-not (Read-SpectreConfirm -Message "Download $($files.Count) file(s) now?" -DefaultAnswer 'y')) { return "[$script:PSMM_ColMute]download cancelled[/]" }
...
function script:Confirm-PSMMCloudHydration {
    ...
    Write-PSMMLine "[$script:PSMM_ColMute]they must be downloaded before loading; this can take a while on a slow connection[/]"
...
function script:Set-PSMMPrimaryLocationUI {
    ...
    Write-PSMMLine "[$script:PSMM_ColWarn]caveat (documented): Install-Module / Install-PSResource still INSTALL to the[/]"
