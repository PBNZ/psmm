...
function script:Build-PSMMFilesView {
    ...
    for ($i = $vp.First; $i -le $vp.Last; $i++) {
        ...
        $on = if ($m.Enabled) { '[green]on[/]' } else { '[red]off[/]' }
        $rw = if ($m.Writable) { 'rw' } else { 'ro' }
        $fl = if ($m.IncludesIgnored) { '[yellow]![/]' } else { ' ' }
    ...
    if ($n) {
        ...
        if ($cur.IncludesIgnored) { $items.Add([Spectre.Console.Markup]::new('[yellow]! this file has an Includes section that is being ignored (main config only)[/]')) }
    }
    ...
    foreach ($w in @(Get-PSMMWarning | Select-Object -First 4)) { $items.Add([Spectre.Console.Markup]::new("[yellow]$(ConvertTo-PSMMSafe $w)[/]")) }
        ...
        Invoke-PSMMLive -Body {
            ...
            while ($true) {
                ...
                switch ($k.Key) {
                    ([ConsoleKey]::Spacebar) {
                        $m = $metas[$st.Cursor]
                        if ($m.Kind -eq 'inline' -or -not $m.Writable) { $st.Status = '[red]read-only - cannot toggle[/]' }
                        else {
                            $m.Enabled = -not $m.Enabled
                            Save-PSMMFile -Path $m.Path -Entries (Get-PSMMAllEntries)
                            $script:PSMM_UI.Dirty = $true
                            $st.Status = "[green]saved ($(if ($m.Enabled) { 'enabled' } else { 'disabled' })) - 'a' applies load/unload changes[/]"
                        }
                        continue
                    }
...
function script:Invoke-PSMMApply {
    ...
    foreach ($e in $activeEntries) {
        if ($e.Mode -eq 'Load' -and -not (Get-Module -Name $e.Name)) {
            ...
            catch { Write-PSMMLine "[red]  $(ConvertTo-PSMMSafe $_.Exception.Message)[/]" }
        }
    }
    ...
    $ui.Dirty = $true
    Write-PSMMLine "[green]$did change(s) applied.[/]"
    $null = Wait-PSMMKey
}
...
function script:New-PSMMConfigFile {
    ...
    if (Test-Path -LiteralPath $path) { Write-PSMMLine '[red]File already exists - not overwriting.[/]'; $null = Wait-PSMMKey; return }
    ...
    if ($tplPath) {
        ...
        Write-PSMMLine "[green]Created $(ConvertTo-PSMMSafe $path) from the '$tpl' template.[/]"
        Write-PSMMLine "[grey]All template modules are set to Mode=Ignore - flip the ones you want to Load/InstallOnly.[/]"
    } else {
        Get-PSMMExampleConfigJson -IsMain $isMain | Set-Content -LiteralPath $path -Encoding utf8
        Write-PSMMLine "[green]Created $(ConvertTo-PSMMSafe $path) (example content, sample module set to Ignore).[/]"
    }
    if (-not $isMain -and $path -ne $prof) {
        Write-PSMMLine "[yellow]Note: this path is not auto-discovered. Add it to the main config's Includes (or `$PSMM_JsonPath).[/]"
        ...
        if ($mainMeta -and $mainMeta.Writable) {
            if (Read-SpectreConfirm -Message 'Add it to the main config Includes now?' -DefaultAnswer 'y') {
                ...
                Write-PSMMLine '[green]Added to Includes.[/]'
            }
        }
    }
    $script:PSMM_UI.Dirty = $true
...
function script:Move-PSMMConfigFile {
    ...
    if ($Meta.Kind -eq 'inline') { Write-PSMMLine '[red]The inline profile block cannot be moved.[/]'; $null = Wait-PSMMKey; return }
    $dir = Read-SpectreText -Message 'Target folder'
    if ([string]::IsNullOrWhiteSpace($dir)) { return }
    if (-not (Test-Path -LiteralPath $dir -PathType Container)) { Write-PSMMLine '[red]Folder not found.[/]'; $null = Wait-PSMMKey; return }
    $new = Join-Path $dir (Split-Path $Meta.Path -Leaf)
    if (Test-Path -LiteralPath $new) { Write-PSMMLine '[red]A file with that name already exists there.[/]'; $null = Wait-PSMMKey; return }
    try { Move-Item -LiteralPath $Meta.Path -Destination $new -ErrorAction Stop }
    catch { Write-PSMMLine "[red]$(ConvertTo-PSMMSafe $_.Exception.Message)[/]"; $null = Wait-PSMMKey; return }
    Write-PSMMLine "[green]Moved to $(ConvertTo-PSMMSafe $new).[/]"
    ...
    if ($wasIncluded) {
        ...
        Save-PSMMFile -Path $main -Entries (Get-PSMMAllEntries)
        Write-PSMMLine '[green]Main config Includes updated to the new path.[/]'
    } elseif ($Meta.Kind -in 'legacy', 'profile') {
        Write-PSMMLine '[yellow]The new location may not be auto-discovered.[/]'
        if ($mainMeta -and $mainMeta.Writable) {
